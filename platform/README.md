...
The Content Workbench is now the primary editorial/operator surface inside Sentinel operator routes. It turns existing opportunities, plans, inbox items, article QA rows and recommendations into content work cards with lifecycle status, priority, category, next action and linked context. Workflow actions in `platform/workflows/content-workflow-actions.json` now sit above raw commands so operators choose tasks such as start research, generate brief, prepare work package, move to review or refresh monitoring. Actions map to local status transitions, local artefact creation or existing allowlisted API actions. Each action now records a visible output with result state, summary, status change, artefact awareness and suggested next step. The standalone Workbench now uses queue, centre document and right action-context panels. Research and brief actions create browser-local artefact documents under `sentinel.contentArtefacts.v1`, so the operator can review actual research and editorial brief content rather than only status transitions. Review notes are stored locally under `sentinel.reviewNotes.v1` for comments, concerns, questions and draft reminders. Raw terminal fallbacks are hidden as advanced implementation detail and are never the primary result. Status changes are browser-local workflow state in `sentinel.contentWorkbench.v1`; workflow action history is browser-local in `sentinel.workflowActions.v1`. They do not publish articles, generate articles, run arbitrary commands or change SEO scoring. See `docs/SENTINEL_CONTENT_WORKBENCH.md`, `docs/SENTINEL_CONTENT_ARTEFACTS.md`, `docs/SENTINEL_WORKFLOW_ACTIONS.md` and `docs/SENTINEL_OPERATOR_JOURNEY_REASSESSMENT.md`.

The standalone `/sentinel` route now uses a unified dark-first design system. The shell, navigation, work-next surface, Workbench lanes, cards, filters and selected-item panel share one operational visual language: deep navy and graphite backgrounds, soft elevated surfaces, cyan for primary focus, and muted semantic state colours. The layout is full-width and fluid, with a slimmer grid-based left rail, expanding central queue and docked right-hand review panel on larger screens. Infrastructure labels are quieter and secondary, so the operator reads the workspace as content workflow first and platform machinery second.
...
- Content Workbench: article and topic workflow from discovery through review, readiness, publication and monitoring, with task-oriented workflow actions above collapsed manual controls.
...
The `/sentinel` prototype now follows a workflow-first design direction. It uses a wider dark operational workspace, a code-native Sentinel mark, compact status rail, workbench-first navigation, a short work-next strip and focused editorial work cards with a selected-item working panel. The latest journey reassessment makes content work artefact-driven: Research, Brief, Package, Review and Monitoring become visible review surfaces, and the guided next-step panel explains what happened and what decision is next. The first artefact implementation adds a central document workspace where generated research and briefs are readable operational documents. The latest review pass adds document-oriented section rhythm, persistent next-step guidance and local review notes so the artefact surface feels like editorial work rather than dashboard metadata. Operational machinery such as pipelines, console, governance, cadence, workspaces, help and diagnostics remains available, but it is visually secondary to the content queue and artefact review flow. This is a local presentation and orchestration UX refactor only and does not change scoring, API exposure, auth mode or stakeholder routes.

For local Pi-backed standalone testing, run `npm run sentinel:launch`. The helper prints the SSH tunnel command, the Vite launch command and the local `/sentinel` URL, then checks whether local port `4317` and the dev server are reachable. `npm run sentinel:launch -- --tunnel` starts the SSH tunnel in the foreground only when explicitly requested, using a local `127.0.0.1` bind. The helper never starts Vite, exposes the Pi API, changes auth mode, stores passwords or mutates the Pi. See `docs/SENTINEL_LOCAL_OPERATOR_LAUNCH.md`.
