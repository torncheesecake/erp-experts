...
The Content Workbench is now the primary editorial/operator surface inside Sentinel operator routes. It turns existing opportunities, plans, inbox items, article QA rows and recommendations into content work cards with lifecycle status, priority, category, next action and linked context. Workflow actions in `platform/workflows/content-workflow-actions.json` now sit above raw commands so operators choose tasks such as start research, generate brief, move to review or refresh monitoring. Actions map to local status transitions, copy-only manual guidance or existing allowlisted API actions. Each action now records a visible output with result state, summary, status change, artefact awareness, copied or fallback command and suggested next step. Status changes are browser-local workflow state in `sentinel.contentWorkbench.v1`; workflow action history is browser-local in `sentinel.workflowActions.v1`. They do not publish articles, generate articles, run arbitrary commands or change SEO scoring. See `docs/SENTINEL_CONTENT_WORKBENCH.md` and `docs/SENTINEL_WORKFLOW_ACTIONS.md`.

The standalone `/sentinel` route now uses a unified dark-first design system. The shell, navigation, work-next surface, Workbench lanes, cards, filters and selected-item panel share one operational visual language: deep navy and graphite backgrounds, soft elevated surfaces, cyan for primary focus, and muted semantic state colours. Infrastructure labels are quieter and secondary, so the operator reads the workspace as content workflow first and platform machinery second.
