...
Notification payloads are generated by cadence, and can also be prepared separately with:
...
`--daily` is an explicit full-cadence alias. `--operator-only` refreshes state, the operator report and the operator notification payload. `--stakeholder-only` generates the stakeholder report and stakeholder notification payload. `--dry-run` prints what would run and creates nothing.
