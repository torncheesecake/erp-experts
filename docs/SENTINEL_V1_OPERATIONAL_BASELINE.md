...
The current standalone UX direction is workflow-first rather than infrastructure-first. `/sentinel` uses a wider dark operational workspace, compact status rail, slimmer navigation and a more prominent Content Workbench. A code-native Sentinel mark now anchors the product identity, and the first viewport shows a live editorial queue rather than a boxed infrastructure dashboard. Existing orchestration, pipelines, governance, activity, authority, workspaces and diagnostics remain available as supporting systems.
