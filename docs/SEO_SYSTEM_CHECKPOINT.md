...
The standalone design-system direction has shifted from boxed infrastructure dashboard to operational content workspace. `/sentinel` now uses a wider dark canvas, a code-native Sentinel mark, compact status rail, slimmer workbench-first navigation, live editorial queue rows in the first viewport and secondary placement for workspace/help controls. Existing infrastructure, governance, activity, pipelines, console and diagnostics remain available but no longer visually dominate the primary operator surface.
