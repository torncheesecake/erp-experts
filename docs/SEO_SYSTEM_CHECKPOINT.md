...
The standalone design-system direction has shifted from boxed infrastructure dashboard to operational content workspace. `/sentinel` now uses a wider dark canvas, a code-native Sentinel mark, compact status rail, slimmer workbench-first navigation, a short work-next strip and focused editorial work cards with a selected-item working panel. Existing infrastructure, governance, activity, pipelines, console and diagnostics remain available but no longer visually dominate the primary operator surface.
...
The editorial workspace now supports local review notes for operator notes, review comments, concerns, questions and draft reminders. Notes are stored under `sentinel.reviewNotes.v1`, remain private to the operator browser and are not exposed on `/seo-progress`.
...
The latest refinement makes artefact review more document-oriented. The active artefact area now carries persistent next-step guidance and the right panel captures review notes and concerns, reducing the dashboard-panel feel.
